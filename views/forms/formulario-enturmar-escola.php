...
try {
    ...
    if (!empty($nome)) {
        $sql = "SELECT * FROM escola WHERE esc_razao_social LIKE :nome ORDER BY esc_razao_social";
    } else {
            ...
            <tbody>
                <?php if (count($resultados) > 0): ?>
                    ...
                    -->
                                    ...
                                    <form action="/proj_foccus/views/forms/formulario-cad-enturmacao.php" method="GET" class="d-inline">
    <input type="hidden" name="escola_id" value="<?php echo $escola['esc_id']; ?>">
    <button type="submit" class="btn btn-sm" style="background-color: #FFA726; color: white;">Modalidade - Escola</button> <!-- Laranja Claro -->
</form>
<form action="/proj_foccus/views/forms/formulario-enturmar-professor.php" method="GET" class="d-inline">
    <input type="hidden" name="enturma_professor" value="<?php echo $escola['esc_inep']; ?>">
    <input type="hidden" name="return_url" value="<?php echo htmlspecialchars($_SERVER['REQUEST_URI']); ?>">
    <button type="submit" class="btn btn-sm" style="background-color: #64B5F6; color: white;">Alocar Professor</button> <!-- Azul Claro -->
</form>
